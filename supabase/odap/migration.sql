...
begin
 ...
 elsif p_action='GET /api/packet' then select case when p_payload->>'metadata_only'='true' then jsonb_build_object('id',p.id,'student_id',p.student_id,'title',p.title,'status',p.status,'created_at',p.created_at,'files',p.files,'verified',p.verified) else to_jsonb(p) end into result from public.odap_packets p where id=(p_payload->>'id')::uuid;return result;
