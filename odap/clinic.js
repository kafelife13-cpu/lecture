...
async function showClinic(){
 ...
 $('clinic-results').innerHTML=`<section class="card"><h2>${E(b.title)}</h2><p>전체 응시 이력 판정 ${b.graded}건 · 오답 ${b.wrongs.length}문항 · 미응답/판정 불가 ${b.unknown}건</p><p class="notice">${E(b.notice)}</p>${clinicKeyWarnings(b)}<div class="row"><button id="clinic-tags">문항 개념·유형 분류</button><button id="clinic-reload">제작 상태 새로고침</button><button id="clinic-print-all">선택한 자료 묶어 인쇄 · PDF</button></div></section>`+
