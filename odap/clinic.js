...
let clinicExam='all',clinicRun=null,clinicBusy=false,clinicRequest=null;
...
async function showClinic(){
 ...
 const b=clinicRun.body,c=clinicCounts(b),packetGroups=[b.packet_ids||[b.packet_id],[],[],b.type_packets||[],b.concept_packets||[]],packets=await Promise.all(packetGroups.flat().map(id=>api('/api/packet?id='+id))),jobs=await api('/api/clinic?action=jobs&student_id='+encodeURIComponent(sid));
 ...
 $('clinic-results').innerHTML=`<section class="card"><h2>${E(b.title)}</h2><p>전체 응시 이력 판정 ${b.graded}건 · 오답 ${b.wrongs.length}문항 · 미응답/판정 불가 ${b.unknown}건</p><p class="notice">${E(b.notice)}</p><div class="row"><button id="clinic-tags">문항 개념·유형 분류</button><button id="clinic-reload">제작 상태 새로고침</button><button id="clinic-print-all">선택한 자료 묶어 인쇄 · PDF</button></div></section>`+
 [0,3,4].map(i=>{const name=clinicNames[i];return `<section class="card"><div class="card-head"><h2>${[0,3,4].indexOf(i)+1}. ${name}</h2>${badge(status[i],i===0&&native?'green':'amber')}</div><p>${c[i]}${i===1?'개 오답 분석':'문항'}</p>${i===0?'<p>원문 표·밑줄·옛한글과 정답을 확인한 뒤 배부하세요.</p>':i===1?`<p>세부 개념 미분류 ${b.unclassified}개. 학생 설명이 없는 원인은 임의로 확정하지 않았습니다.</p>`:i===2?'<p>학교가 일치하는 검수된 학교 프린트 근거만 사용합니다. 현재 규칙 OX는 참 명제이며, 빈칸은 근거 문장 속 지정어를 사용합니다.</p>':'<p>검수 완료·고난도·동일 태그 조건으로 선정하며 유형/개념 과제 사이의 문항 중복을 제외합니다.</p>'}<div class="row"><label class="check"><input type="checkbox" data-clinic-part="${i}" ${c[i]?'checked':''}>묶음 출력에 포함</label><button data-clinic-view="${i}">결과 · 부족 내역 보기</button>${packets.filter(p=>packetGroups[i].includes(p.id)).flatMap(p=>(p.files||[]).map(f=>`<button data-clinic-packet="${p.id}" data-clinic-file="${E(f)}">${E(f.split('.').pop().toUpperCase())} 다운로드</button>`)).join('')}</div>${jobs.filter(j=>packetGroups[i].includes(j.packet_id)&&j.status==='failed').map(j=>`<p class="notice">한글 제작 실패: ${E(j.error)}</p>`).join('')}</section>`;}).join('');
...
function clinicPreview(parts){
 ...
 const sections=parts.map(i=>{
  let text='';
  if(i===0)text='<h2>오답 유형 분석</h2>'+b.diagnosis.map(d=>`<section><h3>${E(d.exam)} · ${E(d.num)}번</h3><p>세부 개념: ${E(d.concepts.join(' / ')||'분류 필요')}</p><p>${E(d.cause)}</p></section>`).join('')+'<h2>전체 누적 오답 문제</h2>'+qs(b.wrongs)+keys(b.wrongs);
  ...
  return `<section class="clinic-print-part"><h1>${clinicNames[i]}</h1>${text}</section>`;
 }).join('');
...
navigate=async function(name){if(clinicBusy){toast('제작 결과를 저장 중입니다. 잠시 기다려 주세요.');return;}if(name!=='clinic')return navigateBeforeClinic(name);page=name;charts.forEach(c=>c.destroy());charts=[];$('breadcrumb').textContent=titles.clinic;document.querySelectorAll('[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===name));$('main').innerHTML='<div class="loading">학생의 제작 기록을 불러옵니다…</div>';try{await clinicPage();}catch(e){$('main').innerHTML=empty('제작 화면을 불러오지 못했습니다',e.message);}};
