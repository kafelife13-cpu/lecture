...
async function showClinic(){
 ...
 document.querySelectorAll('[data-clinic-retry]').forEach(btn=>btn.onclick=()=>action(async()=>{await api('/api/clinic',{action:'retry',student_id:who,packet_id:btn.dataset.clinicRetry});if(sid===who&&clinicRun===run)await showClinic();},btn));
