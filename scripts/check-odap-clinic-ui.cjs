...
const c={clinicRun:run,sid:'s1',page:'clinic',clinicBusy:false,clinicPoll:null,clinicViewSequence:0,clinicNames:['Original','','','Type','Concept'],clinicCounts:()=>[2,0,0,0,0],clinicKeyWarnings:()=>'',E:String,badge:x=>x,on:()=>{},clinicTagEditor:()=>{},clinicPreview:()=>{},action:()=>{},document:{querySelectorAll:()=>[],querySelector:()=>null},$:id=>elements[id],clearTimeout:()=>{timer=null},setTimeout:(fn,ms)=>{timer={fn,ms};return 1;},api:async url=>{if(apiGate)await apiGate;return url.includes('action=jobs')?jobs:{id:'p1',files:[]};}};
