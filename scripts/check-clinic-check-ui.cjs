...
(async()=>{
 ...
 assert.equal(await page.locator('[data-check-id]').count(),1);
 assert.equal(await page.locator('#clinic-check-slot option').allTextContents().then(x=>x.join(',')),'모든 요일,수요일');
 ...
 await page.locator('#clinic-check-filter').selectOption('unchecked');assert.equal(await page.locator('[data-check-id]').count(),0);
