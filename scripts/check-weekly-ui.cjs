...
(async()=>{
 ...
 await page.evaluate(()=>{
  ...
  window.renderStudentClassAttendance=()=>{};window.refreshSokTask=()=>{};window.renderWeeklyExtras=()=>{};
  ...
  window.sb={rpc:async(name,args)=>{
   ...
   if(table==='exams')return {select:()=>({eq:()=>({order:async()=>({data:window.fixtureExams})})})};
 ...
 assert.equal(await page.locator('.weekly-task').count(),4);
 ...
 assert.equal(await page.locator('[data-task="vocab"], [data-task="concept"]').count(),0);
 ...
 assert.equal(await page.locator('#weekly-create-form [name="questions"], #weekly-create-form [name="vocab_units"]').count(),0);
 ...
 assert.equal(await page.locator('[data-weekly-action="browse-task"]').count(),4);
 ...
 console.log('PASS: four tasks, mobile layout, retired OX/word entries absent, notebook photo submission, teacher table/review');
