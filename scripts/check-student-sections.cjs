...
(async()=>{
 ...
 await c.renderSExam('clinic');assert.equal(c.sExamClinicCache.length,1);assert.equal(els['s-exam-list-homework'].closest().hidden,true);assert.equal(els['s-exam-list-homework'].innerHTML,'');
 await c.renderSExam('homework');assert.equal(c.sExamClinicCache.length,0);assert.equal(els['s-exam-list-homework'].innerHTML,'homework');assert.equal(els['s-exam-clinic-groups'].closest().hidden,true);
