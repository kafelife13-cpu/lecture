(function(root){
 ...
 async function json(file){const r=await fetch(base+file+'?v=20260908-detail1');if(!r.ok)throw Error('자료를 불러오지 못했습니다 ('+r.status+').');return r.json();}
 ...
 function links(n,solutions){const prefix=base+'round-'+String(n).padStart(2,'0');return '<a class="btn" target="_blank" rel="noopener" href="'+prefix+'-questions.pdf">문제지 PDF</a>'+(solutions?'<a class="btn" target="_blank" rel="noopener" href="'+prefix+'-explanations.pdf">원본 해설 PDF</a><a class="btn" target="_blank" rel="noopener" href="'+prefix+'-corrected.pdf">교정 해설 PDF</a><a class="btn" download href="'+prefix+'-corrected.txt">교정 해설 TXT</a>':'');}
 root.renderLiterature=async function(){
  ...
  try{
   ...
   container.innerHTML=(teacher?'<div class="card lit-intro"><strong>10회 · 170문항 · 회당 30분</strong><p>문제지, 문항별 정답과 상세 해설, 작품 해제, 원문 해설, 문학 개념을 함께 등록합니다.</p><button class="btn blue" id="lit-register" onclick="registerLiterature()"'+(existing.size===10?' disabled':'')+'>'+ (existing.size===10?'10회 등록 완료':'미등록 회차 등록')+'</button><span id="lit-import-status" role="status"> '+existing.size+'/10회 등록됨</span></div>':'<p class="lit-intro">회당 17문항 · 30분 · 38점 만점 (성적은 100점 환산)<br>문제 풀기 → 답안 제출 → 오답 이유 쓰기 → 다시 풀고 해설 확인</p>')+
 ...
 root.renderLiteratureFeedback=function(wrong){
  document.getElementById('lit-feedback')?.remove();document.getElementById('s-res-wrong-list')?.classList.toggle('old-hangul',isLiteratureExam(sExamData));if(!isLiteratureExam(sExamData))return;
  ...
  el.innerHTML='<h3>문학 풀이 피드백</h3><p>'+(!wrong.length?'모든 문항을 맞혔어요. 해설의 근거와 내 풀이를 비교해 보세요.':wrong.length+'문항을 다시 확인해 보세요. '+wrong.filter(w=>w.my==='미선택').length+'문항은 답이 선택되지 않았어요.')+'</p>'+[...types].map(([t,n])=>'<p><strong>'+esc(t)+' · '+n+'문항</strong><br>'+(/표현|서술/.test(t)?'선택지의 표현 기법이 실제로 나타나는 구절을 찾고, 그 효과까지 지문과 맞는지 확인하세요.':/인물|사건/.test(t)?'누가 어떤 상황에서 말하고 행동했는지, 사건 전후의 변화를 함께 확인하세요.':/보기|감상|외적/.test(t)?'보기의 관점을 먼저 정리한 뒤, 선택지의 해석을 뒷받침하는 지문 근거를 찾으세요.':'선택지의 대상·상황·태도를 각각 지문과 비교하고, 틀린 선택지의 어긋난 표현을 짚어보세요.')+'</p>').join('')+'<details><summary>전체 문항 정답·상세 해설 확인</summary>'+sExamData.questions.map((q,i)=>'<details><summary>'+q.num+'번 · '+(wrong.some(w=>w.idx===i)?'다시 확인':'정답')+' · 정답 '+q.answer+'</summary><pre>'+esc(q.detailedReviewed?q.detailedExplanation:q.explanation)+'</pre>'+literatureQuestionResources(q)+'</details>').join('')+'</details><p>아래 오답노트에 틀린 이유를 적고 다시 답을 선택한 뒤 저장하세요.</p>';
  document.querySelector('.lit-exam-tools')?.after(el);
 };
 root.showLiteratureText=async function(n){
  try{const d=await data(n),el=document.getElementById('lit-detail');el.hidden=false;el.innerHTML='<h2>'+n+'회 지문·문제 텍스트</h2><p>옛한글·기호는 동일 교재의 원본 글꼴과 대조해 복원했습니다. 원래 배치와 밑줄은 문제지 PDF에서 확인할 수 있어요.</p>'+d.problemPages.map(p=>'<details><summary>원본 '+p.page+'쪽</summary><pre>'+esc(p.text)+'</pre></details>').join('');el.scrollIntoView({behavior:'smooth'});}catch(e){notify(e.message,'error');}
 };
 root.showLiteratureSolutions=async function(n){
  if(session?.role!=='teacher'||studentPreviewMode)return;
  try{const d=await data(n),el=document.getElementById('lit-detail');el.hidden=false;el.innerHTML='<h2>'+n+'회 정답·해설</h2>'+links(n,true)+'<details><summary>작품 해제·특징·주제</summary>'+(d.works||[]).map(w=>'<pre>'+esc(w.text)+'</pre>').join('')+'</details>'+d.exam.questions.map(q=>'<details><summary>'+q.num+'번 · 정답 '+q.answer+' · '+q.type+'</summary><pre>'+esc(q.detailedReviewed?q.detailedExplanation:q.explanation)+'</pre><p>원본 '+(q.explanationPages||[q.explanationPage]).join(', ')+'쪽 · 원본 대조 교정</p></details>').join('');el.scrollIntoView({behavior:'smooth'});}catch(e){notify(e.message,'error');}
 };
