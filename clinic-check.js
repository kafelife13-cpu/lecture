...
(function(root){
 ...
 const dayOrder=['수','목','금','주말','월','화','요일 미확인'];
 function checkoutDay(row){if(!row.slot_id)return null;const match=String(row.slot_label||'').match(/(?:^|[·\s])(주말|[월화수목금토일])(?:요일)?(?=[·\s]|$)/);return match?(['토','일','주말'].includes(match[1])?'주말':match[1]):'요일 미확인';}
 if(typeof module!=='undefined')module.exports={modes,monday,summary,checkoutDay,dayOrder};
 ...
 root.renderClinicCheckout=async function(){
  ...
  try{
   ...
   rows=result.filter(r=>checkoutDay(r)!==null);loadedWeek=week;owner=who;
   const select=el('clinic-check-slot'),old=select.value;
   const slots=dayOrder.filter(day=>rows.some(r=>checkoutDay(r)===day)).map(day=>[day,day==='주말'||day==='요일 미확인'?day:day+'요일']);
   select.innerHTML='<option value="all">모든 요일</option>'+slots.map(([id,label])=>'<option value="'+esc(id)+'">'+esc(label)+'</option>').join('');
 ...
 function render(){
  ...
  const selected=rows.filter(r=>(slot==='all'||checkoutDay(r)===slot)&&(!query||r.student_name.includes(query)));
  ...
  const groups=new Map();shown.forEach(r=>{const key=checkoutDay(r);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(r);});
  el('clinic-check-body').innerHTML=[...groups].sort((a,b)=>dayOrder.indexOf(a[0])-dayOrder.indexOf(b[0])).map(([key,list])=>{
   const total=summary(selected.filter(r=>checkoutDay(r)===key));
   return '<section class="clinic-check-group"><h3>'+esc(key==='주말'||key==='요일 미확인'?key:key+'요일')+'</h3><p>전체 '+total.total+'명 · 확인 '+total.checked+'명 · 미확인 '+(total.total-total.checked)+'명</p>'+list.sort((a,b)=>(a.mode!=='unchecked')-(b.mode!=='unchecked')||a.student_name.localeCompare(b.student_name,'ko')).map(r=>
